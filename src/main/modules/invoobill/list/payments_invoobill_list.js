angular.module('raiffeisen-payments')
    .config(function (pathServiceProvider, stateServiceProvider) {
        stateServiceProvider.state('payments.invoobill.list', {
            ...
            data: {
                analyticsTitle: "raiff.payments.invoobill.label"
            }
        });
    })
    .controller('PaymentsInvoobillListController', function ($scope, $state, bdTableConfig, $timeout, $q, translate, invoobillPaymentsService, paymentsService, $filter, parameters, pathService, viewStateService, lodash, rbPaymentTypes, standingTransferService, STANDING_FREQUENCY_TYPES, rbPaymentOperationTypes, initialState) {
        $scope.dateRange = {};
        ...
        $scope.resolveTemplateType = function (transferType) {
            return "{0}/modules/invoobill/list/details/{1}_invoobill_payment_details.html".format(pathService.generateTemplatePath("raiffeisen-payments"), transferType.toLowerCase());
        };
        ...
        $scope.onEdit = function(payment) {
            if (payment.transferType == rbPaymentTypes.STANDING.code) {
                ...
                viewStateService.setInitialState('payments.invoobill.manage.edit', {
                    referenceId: payment.id
                });
                $state.go('payments.invoobill.manage.edit.fill');
            }
        };
        ...
        $scope.onDelete = function(payment) {
            if (payment.transferType == rbPaymentTypes.STANDING.code) {
                ...
                paymentsService.remove(responseObject).then(function(resp) {
                    ...
                    viewStateService.setInitialState('payments.invoobill.manage.delete', {
                        paymentId: payment.id,
                        referenceId: referenceId,
                        paymentDetails: payment
                    });
                    $state.go('payments.invoobill.manage.delete.fill');
                });
            }
        };
            ...
            tableData: {
                getData: function (defer, $params) {
                    ...
                    if(initialState && initialState.relation === 'DETAILS_FROM_WIDGET'){
                        ...
                        viewStateService.resetInitialState('payments.invoobill.list',{});
                        formSummary(summary);
