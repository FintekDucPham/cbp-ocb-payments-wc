angular.module('raiffeisen-payments')
    .config(function (pathServiceProvider, stateServiceProvider) {
        stateServiceProvider.state('payments.multisign.list', {
            url: "/list",
            templateUrl: pathServiceProvider.generateTemplatePath("raiffeisen-payments") + "/modules/multisign_list/list/payments_future_list.html",
            controller: "PaymentsMultisgnPaymentsListController",
            resolve: {
    ...
    .controller('PaymentsMultisgnPaymentsListController', function ($scope, $state, bdTableConfig, $timeout, $q, translate, multisignService, $filter, parameters, pathService, viewStateService, lodash, rbPaymentTypes, standingTransferService, STANDING_FREQUENCY_TYPES, rbPaymentOperationTypes, initialState) {
        $scope.dateRange = {};
            ...
            tableData: {
                getData: function (defer, $params) {
                    ...
                    if(initialState && initialState.relation === 'DETAILS_FROM_WIDGET'){
                        ...
                        multisignService.search({}).then(function (response) {
                            var summary = {};
                            var paymentsList = [];
                            _.each(response.content, function(payment) {
                                ...
                                paymentsList.push(payment.payment);
                                addPaymentAmountToSummary(payment.payment, summary);
                                //linkDetailsLoading(payment.payment);
                            });
                            formSummary(summary);
                            defer.resolve(paymentsList);
