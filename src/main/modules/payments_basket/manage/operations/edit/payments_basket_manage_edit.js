angular.module('raiffeisen-payments')
    .config(function (pathServiceProvider, stateServiceProvider) {
        stateServiceProvider.state('payments.basket.manage.edit', {
            url: "/edit",
            abstract: true,
            templateUrl: pathServiceProvider.generateTemplatePath("raiffeisen-payments") + "/modules/payments_basket/manage/operations/edit/payments_basket_manage_edit.html",
            controller: "PaymentsBasketManageEditController",
            data: {
                analyticsTitle: "raiff.payments.future.edit.label"
            }
        }).state('payments.basket.manage.edit.fill', {
            url: "/fill",
        ...
        }).state('payments.basket.manage.edit.verify', {
            url: "/verify",
            templateUrl: function ($stateParams) {
                return pathServiceProvider.generateTemplatePath("raiffeisen-payments") + "/modules/new/verify/payments_new_verify.html";
            },
            controller: 'PaymentBasketManageEditVerifyController',
            data: {
                analyticsTitle: "config.multistepform.labels.step2"
            }
        }).state('payments.basket.manage.edit.status', {
            url: "/status",
    ...
    .controller('PaymentsBasketManageEditController', function ($scope, $q, lodash, insuranceAccounts, formService, recipientManager, recipientGeneralService, authorizationService, $stateParams, paymentsService, rbPaymentOperationTypes, rbPaymentTypes, initialState, zusPaymentInsurances, RECIPIENT_IDENTITY_TYPES) {
        ...
        //dispatch
        $scope.payment.operation = rbPaymentOperationTypes.NEW;
        ...
        $scope.payment.initData.promise = paymentsService.get(initialState.referenceId+"@basket_edit", {}).then(function(data){
