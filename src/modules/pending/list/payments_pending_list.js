angular.module('ocb-payments')
    ...
    .controller('PaymentsPendingListController', function ($scope, $q, $timeout, bdTableConfig, translate, parameters, paymentsService, lodash, $state, $stateParams, $filter) {
        ...
        var calculateModelDate = function() {
            var lastMiliseconds = $scope.pendingList.filterData.last.value * 24 * 3600 * 1000;

            switch($scope.pendingList.filterData.last.type.selected) {
            ...
            $scope.pendingList.filterData.last.dateFrom = new Date((+new Date()) - lastMiliseconds);
        };

        $scope.onFilterLastTypeChange = function() {
            calculateModelDate();

            var modelDate = $scope.pendingList.filterData.last.dateFrom,
                minDate   = $scope.pendingList.minDate,
            ...
            if (modelDate.getTime() < minDate.getTime()) {
                switch ($scope.pendingList.filterData.last.type.selected) {
                    case LAST_TYPES.WEEKS:
                    {
                        $scope.pendingList.filterData.last.value = Math.floor(diffMS / (1000 * 3600 * 24 * 7));
                        break;
                    }
                    case LAST_TYPES.MONTH:
                    {
                        $scope.pendingList.filterData.last.value = parameters.detal.max;
                        break;
                    }
                }
            }
            ...
            $scope.pendingList.filterData.periodType.model = PERIOD_TYPES.LAST;
        };
        ...
        if (parameters.customerDetails.context === 'MICRO') {
            $scope.pendingList.filterData.last.value = now.getDate(); // bo data bieżąca - data początku miesiąca + 1 to taki skomplikowany sposób na powiedzenie, że chodzi o dzień miesiąca
            $scope.pendingList.filterData.last.default = now.getDate();
            $scope.pendingList.filterData.range.dateFrom = firstDayOfCurrentMonth;

            // hello world, tutaj weeeee ned to change something, i hope only here ;)
            $scope.pendingList.minDate = new Date((new Date()).setMonth(now.getMonth() - parameters.micro.max));

            $scope.pendingList.maxOffset = parameters.micro.max;
        }
